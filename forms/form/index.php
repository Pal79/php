...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST["name"]) && !empty($_POST["name"]) && $_POST["name"] == stringTest($_POST["name"]) && preg_match("/^[a-zA-ZáéíóöőúüűÁÉÍÓÖŐÚÜŰ\s]*$/u", $_POST["name"]) && strlen($_POST["name"]) <= 40) {
        $name = $_POST["name"];
    } else {
        $nameErr = "Hibás név!!!";
    }
    if(isset($_POST["userName"]) && !empty($_POST["userName"]) && $_POST["userName"] == stringTest($_POST["userName"]) && preg_match("/^[a-zA-Z\s\-]*$/", $_POST["userName"]) && strlen($_POST["userName"]) <= 40) {
